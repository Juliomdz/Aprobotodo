...
import { addDoc, collection, getDocs, setDoc, doc, updateDoc, getFirestore, QuerySnapshot, onSnapshot } from 'firebase/firestore';
...
export class PhotoService 
{
   storage = getStorage();
   db = getFirestore(app);
   cosasLindas = collection(this.db, "cosaslindas");
   cosasFeas = collection(this.db, "cosasfeas");
  ...
  observeCosasFeasChanges(callback: (snapshot: QuerySnapshot) => void) {
    onSnapshot(this.cosasFeas, callback);
  }
  observeCosasLindasChanges(callback: (snapshot: QuerySnapshot) => void) {
    onSnapshot(this.cosasLindas, callback);
  }
  ...
  private async subirFoto(filePhoto:any,fotoEstructuradaRecibida:any)
  {
    ...
    let referencia = ref(this.storage, `images/${fotoEstructuradaRecibida.emisor + "-" + fechaValida + "-" + fotoEstructuradaRecibida.hora}`);
    ...
    uploadString(referencia, filePhoto.dataUrl,'data_url').then((snapshot)=>
    {
      getDownloadURL(referencia).then(async (url)=>
      {
          ...
          let newDocument = doc(this.db,"cosaslindas", newID.toString());
  ...
  async leerDBCosasLindas()
  {
    ...
    const querySnapshot = await getDocs(collection(this.db, "cosaslindas"));
  ...
  {
    let querySnapshot = getDocs(this.cosasLindas);
    let flagMax = 0;
  ...
  {
    let docRef = doc(this.db, `cosaslindas/${foto.id}`);
  ...
  private async subirFotoFea(filePhoto:any,fotoEstructuradaRecibida:any)
  {
    ...
    uploadString(referencia, filePhoto.dataUrl,'data_url').then((snapshot)=>
    {
      ...
      {
          fotoEstructuradaRecibida.foto = url;
          let newDocument = doc(this.db,"cosasfeas", newID.toString());
          await setDoc(newDocument,fotoEstructuradaRecibida);
      });
    }).catch( () => { console.log("Error");})   
  }
  ...
  async leerDBCosasFeas()
  {
    ...
    const querySnapshot = await getDocs(collection(this.db, "cosasfeas"));
  ...
  {
    let querySnapshot = getDocs(this.cosasFeas);
    let flagMax = 0;
  ...
  {
    let docRef = doc(this.db, `cosasfeas/${foto.id}`);
    return updateDoc(docRef, foto);
  }
}
